...
import com.gmm.gctall.potion.PotionChanneling;
...
public final class ProcedureProZethurSkill {
  ...
  private ProcedureProZethurSkill() {
  }
  ...
  public static void executeProcedure(Map<String, Object> dependencies) {
    ...
    ProcedureEffectHelper.applyToEntities(Entity.class, world, center, 64.0D, entity -> {
      ResourceLocation name = EntityList.getKey(entity);
      if (name != null && "aether_legacy".equals(name.getNamespace()) && "zephyr".equals(name.getPath()) && entity instanceof EntityLivingBase) {
    ...
    if (world.rand.nextDouble() < 0.001D)
      ProcedureEffectHelper.addEffectToPlayers(world, center, 64.0D, PotionChanneling.potion, 200, 0, false, true);
  ...
  private static void teleportPlayersToGroundBelow(World world, BlockPos center, double triggerRadius) {
    if (world == null || world.isRemote)
      return;
    for (EntityPlayer player : ProcedureEffectHelper.playersWithin(world, center, triggerRadius)) {
