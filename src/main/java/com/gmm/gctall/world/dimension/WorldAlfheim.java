...
import com.gmm.gctall.block.BlockAlfStone;
import com.gmm.gctall.block.BlockOrichalcosDreamwood;
...
@Tag
public class WorldAlfheim extends GctAllElement {
  public static int DIMID = 42;
  
  public static final boolean NETHER_TYPE = false;
  
  @ObjectHolder("gct_all:alfheim_portal")
  public static final BlockCustomPortal portal = null;
  
  public static DimensionType dtype;
  
  public WorldAlfheim(GctAllContent instance) {
    super(instance, 223);
  }
  ...
  public void initElements() {
    ...
    this.elements.items.add(() -> (Item)(new ItemAlfheim()).setTranslationKey("alfheim").setRegistryName("alfheim"));
  }
  
  public void preInit(FMLPreInitializationEvent event) {
    if (DimensionManager.isDimensionRegistered(DIMID)) {
      DIMID = DimensionManager.getNextFreeDimId();
      System.err.println("Dimension ID for dimension alfheim is already registered. Falling back to ID: " + DIMID);
    } 
    dtype = DimensionType.register("alfheim", "_alfheim", DIMID, WorldProviderMod.class, false);
    DimensionManager.registerDimension(DIMID, dtype);
  }
  
  @SideOnly(Side.CLIENT)
  public void registerModels(ModelRegistryEvent event) {
    RegistrationHelper.registerItemModel(ItemAlfheim.block, "alfheim");
  }
  
  public static class WorldProviderMod extends WorldProvider {
    public void init() {
      this.biomeProvider = new WorldAlfheim.BiomeProviderCustom(this.world.getSeed());
      this.nether = false;
      this.hasSkyLight = true;
    }
    
    public DimensionType getDimensionType() {
      return WorldAlfheim.dtype;
    }
    
    @SideOnly(Side.CLIENT)
    public Vec3d getFogColor(float par1, float par2) {
      return new Vec3d(1.0D, 0.8D, 1.0D);
    }
    
    public IChunkGenerator createChunkGenerator() {
      return new WorldAlfheim.ChunkProviderModded(this.world, this.world.getSeed() - WorldAlfheim.DIMID);
    }
  ...
  public static class TeleporterDimensionMod extends Teleporter {
        ...
        for (int j7 = -1; j7 <= 1; j7++) {
          for (int l7 = 1; l7 < 3; l7++) {
            for (int k8 = -1; k8 < 3; k8++) {
              ...
              boolean flag = (k8 < 0);
              this.world.setBlockState(new BlockPos(k9, k10, k11), flag ? BlockOrichalcosDreamwood.block
                  
                  .getDefaultState().getBlock().getDefaultState() : Blocks.AIR
                  .getDefaultState());
            } 
          } 
        } 
      } 
      IBlockState iblockstate = WorldAlfheim.portal.getDefaultState().withProperty((IProperty)BlockPortal.AXIS, (l6 == 0) ? (Comparable)EnumFacing.Axis.Z : (Comparable)EnumFacing.Axis.X);
      for (int i8 = 0; i8 < 4; i8++) {
        for (int l8 = 0; l8 < 4; l8++) {
          for (int l9 = -1; l9 < 4; l9++) {
            ...
            this.world.setBlockState(new BlockPos(l10, l11, k12), flag1 ? BlockOrichalcosDreamwood.block
        ...
        for (int j1 = -2; j1 <= 2; j1++) {
          for (int k1 = -2; k1 <= 2; k1++) {
            for (int l1 = -1; l1 < 3; l1++) {
              ...
              int k2 = k + k1 * 0 - j1 * 1;
              boolean flag = (l1 < 0);
              this.world.setBlockState(new BlockPos(i2, j2, k2), flag ? BlockOrichalcosDreamwood.block
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw) {
        ...
        for (int i1 = -128; i1 <= 128; i1++) {
          for (int j1 = -128; j1 <= 128; j1++) {
            ...
            for (; blockpos1.getY() >= 0; blockpos1 = blockpos1.down()) {
              ...
              if (this.world.getBlockState(blockpos1).getBlock() == WorldAlfheim.portal) {
                for (blockpos2 = blockpos1.down(); this.world.getBlockState(blockpos2).getBlock() == WorldAlfheim.portal; 
      ...
      if (d0 >= 0.0D) {
        ...
        BlockPattern.PatternHelper blockpattern$patternhelper = WorldAlfheim.portal.createPatternHelper((World)this.world, blockpos);
  ...
  public static class BlockCustomPortal extends BlockPortal {
    public BlockCustomPortal() {
      setHardness(-1.0F);
      setTranslationKey("alfheim_portal");
      setRegistryName("alfheim_portal");
      setLightLevel(1.0F);
    }
    ...
    public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random random) {
      for (int i = 0; i < 4; i++) {
        ...
        world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, px, py, pz, vx, vy, vz, new int[0]);
      } 
    }
    ...
    private WorldAlfheim.TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, int dimid) {
      BlockPattern.PatternHelper bph = WorldAlfheim.portal.createPatternHelper(entity.world, new BlockPos((Vec3i)pos));
      ...
      d1 = Math.abs(MathHelper.pct(d1 - ((bph.getForwards().rotateY().getAxisDirection() == EnumFacing.AxisDirection.NEGATIVE) ? 1 : 0), d0, d0 - bph
            .getWidth()));
      double d2 = MathHelper.pct(entity.posY - 1.0D, bph.getFrontTopLeft().getY(), (bph
          .getFrontTopLeft().getY() - bph.getHeight()));
      return new WorldAlfheim.TeleporterDimensionMod(entity.getServer().getWorld(dimid), new Vec3d(d1, d2, 0.0D), bph.getForwards());
    }
    ...
    public static class Size {
      ...
      protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
        ...
        for (i = 0; i < 22; i++) {
          ...
          if (!isEmptyBlock(this.world.getBlockState(blockpos).getBlock()) || this.world
            .getBlockState(blockpos.down()).getBlock() != BlockOrichalcosDreamwood.block.getDefaultState().getBlock())
            break; 
        } 
        Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
        return (block == BlockOrichalcosDreamwood.block.getDefaultState().getBlock()) ? i : 0;
      }
      ...
      protected int calculatePortalHeight() {
        label38: for (this.height = 0; this.height < 21; this.height++) {
          for (int i = 0; i < this.width; i++) {
            ...
            if (!isEmptyBlock(block))
              break label38; 
            if (block == WorldAlfheim.portal)
              this.portalBlockCount++; 
            if (i == 0) {
              block = this.world.getBlockState(blockpos.offset(this.leftDir)).getBlock();
              if (block != BlockOrichalcosDreamwood.block.getDefaultState().getBlock())
                break label38; 
        ...
        for (int j = 0; j < this.width; j++) {
          if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height))
            .getBlock() != BlockOrichalcosDreamwood.block.getDefaultState().getBlock()) {
            this.height = 0;
            break;
          } 
        } 
      ...
      protected boolean isEmptyBlock(Block blockIn) {
        return (blockIn.getDefaultState().getMaterial() == Material.AIR || blockIn == Blocks.FIRE || blockIn == WorldAlfheim.portal);
      }
      ...
      public void placePortalBlocks() {
        for (int i = 0; i < this.width; i++) {
          ...
          for (int j = 0; j < this.height; j++)
            this.world.setBlockState(blockpos.up(j), WorldAlfheim.portal.getDefaultState().withProperty((IProperty)BlockPortal.AXIS, (Comparable)this.axis), 2); 
        } 
      }
    }
  }
  ...
  public static class ChunkProviderModded implements IChunkGenerator {
    private static final IBlockState STONE = BlockAlfStone.block.getDefaultState();
    
    private static final IBlockState STONE2 = BlockAlfStone.block.getDefaultState();
    ...
    public ChunkProviderModded(World worldIn, long seed) {
      ...
      this.caveGenerator = (MapGenBase)new MapGenCaves() {
          protected boolean canReplaceBlock(IBlockState a, IBlockState b) {
            if (a.getBlock() == WorldAlfheim.ChunkProviderModded.STONE.getBlock())
              return true; 
      ...
      this.ravineGenerator = (MapGenBase)new MapGenRavine() {
          protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
            ...
            if (state.getBlock() == WorldAlfheim.ChunkProviderModded.STONE.getBlock() || state.getBlock() == biome.topBlock.getBlock() || state
  ...
  public static class GenLayerBiomesCustom extends GenLayer {
    private Biome[] allowedBiomes = new Biome[] { (Biome)Biome.REGISTRY.getObject(new ResourceLocation("gct_all:alfheim_plain")), (Biome)Biome.REGISTRY
        .getObject(new ResourceLocation("gct_all:alfheim_plain_hill")), (Biome)Biome.REGISTRY
        .getObject(new ResourceLocation("gct_all:alfheim_desert")), (Biome)Biome.REGISTRY
        .getObject(new ResourceLocation("gct_all:alfheim_desert_hill")), (Biome)Biome.REGISTRY
        .getObject(new ResourceLocation("gct_all:alfheim_desert_richland")), (Biome)Biome.REGISTRY
        .getObject(new ResourceLocation("gct_all:alfheim_forest")), (Biome)Biome.REGISTRY
        .getObject(new ResourceLocation("gct_all:reversed_forest")) };
  ...
  public static class BiomeProviderCustom extends BiomeProvider {
    ...
    private GenLayer[] makeTheWorld(long seed) {
      GenLayer biomes = new WorldAlfheim.GenLayerBiomesCustom(1L);
