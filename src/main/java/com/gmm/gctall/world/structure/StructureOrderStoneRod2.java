...
public class StructureOrderStoneRod2
{
    public void generateWorld(final Random random, final int i2, final int k2, final World world, final int dimID, final IChunkGenerator cg, final IChunkProvider cp) {
        ...
        if (random.nextInt(1000000) + 1 <= 50000) {
            for (int count = random.nextInt(1) + 1, a = 0; a < count; ++a) {
                ...
                if (biome == BiomeOrderBasin.biome) {
                    biomeCriteria = true;
                }
                if (biome == BiomeOrderPlain.biome) {
                    biomeCriteria = true;
                }
                if (biomeCriteria) {
                    if (world.isRemote) {
                    ...
                    StructureGenerationHelper.placeTemplate(world, "order_rod_2", spawnTo, rotation, mirror);
                }
            }
        }
    }
