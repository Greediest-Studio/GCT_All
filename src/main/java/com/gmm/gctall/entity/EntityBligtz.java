...
@Tag
public class EntityBligtz extends GctAllElement {
  public static final int ENTITYID = 11;
  
  public static final int ENTITYID_RANGED = 12;
  
  public EntityBligtz(GctAllContent instance) {
    super(instance, 2);
  }
  
  public void initElements() {
    this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("gct_all", "bligtz"), 11).name("bligtz").tracker(64, 3, true).egg(-3355648, -205).build());
    this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityArrowCustom.class).id(new ResourceLocation("gct_all", "entitybulletbligtz"), 12).name("entitybulletbligtz").tracker(64, 1, true).build());
  }
  ...
  public void preInit(FMLPreInitializationEvent event) {
    ...
    RenderingRegistry.registerEntityRenderingHandler(EntityArrowCustom.class, renderManager -> new RenderSnowball<EntityArrowCustom>(renderManager, null, Minecraft.getMinecraft().getRenderItem()) {
          public ItemStack getStackToRender(EntityBligtz.EntityArrowCustom entity) {
            return new ItemStack(ItemBligtzDust.block, 1);
          }
        });
  }
  ...
  public static class EntityCustom extends EntityMob implements IRangedAttackMob {
    ...
    protected void initEntityAI() {
      ...
      this.tasks.addTask(1, new EntityAIBase() {
            public boolean shouldExecute() {
              if (EntityBligtz.EntityCustom.this.getAttackTarget() != null && !EntityBligtz.EntityCustom.this.getMoveHelper().isUpdating())
                return true; 
              return false;
            }
            
            public boolean shouldContinueExecuting() {
              return (EntityBligtz.EntityCustom.this.getMoveHelper().isUpdating() && EntityBligtz.EntityCustom.this.getAttackTarget() != null && EntityBligtz.EntityCustom.this
                .getAttackTarget().isEntityAlive());
            }
            
            public void startExecuting() {
              EntityLivingBase livingentity = EntityBligtz.EntityCustom.this.getAttackTarget();
              Vec3d vec3d = livingentity.getPositionEyes(1.0F);
              EntityBligtz.EntityCustom.this.moveHelper.setMoveTo(vec3d.x, vec3d.y, vec3d.z, 1.0D);
            }
            
            public void updateTask() {
              EntityLivingBase livingentity = EntityBligtz.EntityCustom.this.getAttackTarget();
              double d0 = EntityBligtz.EntityCustom.this.getDistanceSq((Entity)livingentity);
              if (d0 <= getAttackReachSq(livingentity)) {
                EntityBligtz.EntityCustom.this.attackEntityAsMob((Entity)livingentity);
              } else if (d0 < 16.0D) {
                Vec3d vec3d = livingentity.getPositionEyes(1.0F);
                EntityBligtz.EntityCustom.this.moveHelper.setMoveTo(vec3d.x, vec3d.y, vec3d.z, 1.0D);
              } 
            ...
            protected double getAttackReachSq(EntityLivingBase attackTarget) {
              return EntityBligtz.EntityCustom.this.width * 1.5D * EntityBligtz.EntityCustom.this.height * 1.5D + attackTarget.height;
            }
          });
      this.tasks.addTask(2, (EntityAIBase)new EntityAIWander((EntityCreature)this, 0.8D, 20) {
            protected Vec3d getPosition() {
              Random random = EntityBligtz.EntityCustom.this.getRNG();
              double dir_x = EntityBligtz.EntityCustom.this.posX + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
              double dir_y = EntityBligtz.EntityCustom.this.posY + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
              double dir_z = EntityBligtz.EntityCustom.this.posZ + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float flval) {
      EntityBligtz.EntityArrowCustom entityarrow = new EntityBligtz.EntityArrowCustom(this.world, (EntityLivingBase)this);
