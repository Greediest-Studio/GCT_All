...
public final class EntityBligtz {
  ...
  public static final int ENTITYID_RANGED = 12;  public static void registerEntities(net.minecraftforge.event.RegistryEvent.Register<EntityEntry> event) {
    event.getRegistry().register(EntityEntryBuilder.create().entity(BligtzEntity.class).id(new ResourceLocation("gct_all", "bligtz"), 11).name("bligtz").tracker(64, 3, true).egg(-3355648, -205).build());
    event.getRegistry().register(EntityEntryBuilder.create().entity(BligtzProjectileEntity.class).id(new ResourceLocation("gct_all", "entitybulletbligtz"), 12).name("entitybulletbligtz").tracker(64, 1, true).build());
  }
  ...
  public static void registerRenderers(FMLPreInitializationEvent event) {
    RenderingRegistry.registerEntityRenderingHandler(BligtzEntity.class, renderManager -> new RenderLiving(renderManager, new ModelBligtz(), 0.5F) {
          protected ResourceLocation getEntityTexture(Entity entity) {
            return new ResourceLocation("gct_all:textures/bligtz.png");
          }
        });
    RenderingRegistry.registerEntityRenderingHandler(BligtzProjectileEntity.class, renderManager -> new RenderSnowball<BligtzProjectileEntity>(renderManager, null, Minecraft.getMinecraft().getRenderItem()) {
          public ItemStack getStackToRender(EntityBligtz.BligtzProjectileEntity entity) {
            return new ItemStack(ItemBligtzDust.block);
          }
        });
  }

  public static class BligtzEntity extends EntityMob implements IRangedAttackMob {
    public BligtzEntity(World world) {
      super(world);
      setSize(0.6F, 1.8F);
    ...
    protected void initEntityAI() {
      ...
      this.tasks.addTask(1, new EntityAIBase() {
            public boolean shouldExecute() {
              if (EntityBligtz.BligtzEntity.this.getAttackTarget() != null && !EntityBligtz.BligtzEntity.this.getMoveHelper().isUpdating())
                return true;
              return false;
            }

            public boolean shouldContinueExecuting() {
              return (EntityBligtz.BligtzEntity.this.getMoveHelper().isUpdating() && EntityBligtz.BligtzEntity.this.getAttackTarget() != null && EntityBligtz.BligtzEntity.this
                .getAttackTarget().isEntityAlive());
            }

            public void startExecuting() {
              EntityLivingBase livingentity = EntityBligtz.BligtzEntity.this.getAttackTarget();
              Vec3d vec3d = livingentity.getPositionEyes(1.0F);
              EntityBligtz.BligtzEntity.this.moveHelper.setMoveTo(vec3d.x, vec3d.y, vec3d.z, 1.0D);
            }

            public void updateTask() {
              EntityLivingBase livingentity = EntityBligtz.BligtzEntity.this.getAttackTarget();
              double d0 = EntityBligtz.BligtzEntity.this.getDistanceSq((Entity)livingentity);
              if (d0 <= getAttackReachSq(livingentity)) {
                EntityBligtz.BligtzEntity.this.attackEntityAsMob((Entity)livingentity);
              } else if (d0 < 16.0D) {
                Vec3d vec3d = livingentity.getPositionEyes(1.0F);
                EntityBligtz.BligtzEntity.this.moveHelper.setMoveTo(vec3d.x, vec3d.y, vec3d.z, 1.0D);
              }
            ...
            protected double getAttackReachSq(EntityLivingBase attackTarget) {
              return EntityBligtz.BligtzEntity.this.width * 1.5D * EntityBligtz.BligtzEntity.this.height * 1.5D + attackTarget.height;
            }
          });
      this.tasks.addTask(2, (EntityAIBase)new EntityAIWander((EntityCreature)this, 0.8D, 20) {
            protected Vec3d getPosition() {
              Random random = EntityBligtz.BligtzEntity.this.getRNG();
              double dir_x = EntityBligtz.BligtzEntity.this.posX + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
              double dir_y = EntityBligtz.BligtzEntity.this.posY + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
              double dir_z = EntityBligtz.BligtzEntity.this.posZ + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
    ...
    public SoundEvent getAmbientSound() {
      return (SoundEvent)SoundEvent.REGISTRY.getObject(new ResourceLocation("gct_all:bligtz_br"));
    }

    public SoundEvent getHurtSound(DamageSource ds) {
      return (SoundEvent)SoundEvent.REGISTRY.getObject(new ResourceLocation("gct_all:bligtz_ht"));
    }

    public SoundEvent getDeathSound() {
      return (SoundEvent)SoundEvent.REGISTRY.getObject(new ResourceLocation("gct_all:bligtz_de"));
    }
    ...
    protected ResourceLocation getLootTable() {
      return new ResourceLocation("gct_all:entities/bligtz");
    }
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float flval) {
      EntityBligtz.BligtzProjectileEntity entityarrow = new EntityBligtz.BligtzProjectileEntity(this.world, (EntityLivingBase)this);
  ...
  public static class BligtzProjectileEntity extends EntityTippedArrow {
    public BligtzProjectileEntity(World a) {
      super(a);
    }

    public BligtzProjectileEntity(World worldIn, double x, double y, double z) {
      super(worldIn, x, y, z);
    }

    public BligtzProjectileEntity(World worldIn, EntityLivingBase shooter) {
