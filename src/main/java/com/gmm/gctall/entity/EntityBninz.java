...
@Tag
public class EntityBninz extends GctAllElement {
  public static final int ENTITYID = 13;
  
  public static final int ENTITYID_RANGED = 14;
  
  public EntityBninz(GctAllContent instance) {
    super(instance, 3);
  }
  
  public void initElements() {
    this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityCustom.class).id(new ResourceLocation("gct_all", "bninz"), 13).name("bninz").tracker(64, 3, true).egg(-16777216, -10066330).build());
    this.elements.entities.add(() -> EntityEntryBuilder.create().entity(EntityArrowCustom.class).id(new ResourceLocation("gct_all", "entitybulletbninz"), 14).name("entitybulletbninz").tracker(64, 1, true).build());
  }
  ...
  public void preInit(FMLPreInitializationEvent event) {
    ...
    RenderingRegistry.registerEntityRenderingHandler(EntityArrowCustom.class, renderManager -> new RenderSnowball<EntityArrowCustom>(renderManager, null, Minecraft.getMinecraft().getRenderItem()) {
          public ItemStack getStackToRender(EntityBninz.EntityArrowCustom entity) {
            return new ItemStack(ItemBninzDust.block, 1);
          }
        });
  }
  ...
  public static class EntityCustom extends EntityMob implements IRangedAttackMob {
    ...
    protected void initEntityAI() {
      ...
      this.tasks.addTask(1, new EntityAIBase() {
            public boolean shouldExecute() {
              if (EntityBninz.EntityCustom.this.getAttackTarget() != null && !EntityBninz.EntityCustom.this.getMoveHelper().isUpdating())
                return true; 
              return false;
            }
            
            public boolean shouldContinueExecuting() {
              return (EntityBninz.EntityCustom.this.getMoveHelper().isUpdating() && EntityBninz.EntityCustom.this.getAttackTarget() != null && EntityBninz.EntityCustom.this
                .getAttackTarget().isEntityAlive());
            }
            
            public void startExecuting() {
              EntityLivingBase livingentity = EntityBninz.EntityCustom.this.getAttackTarget();
              Vec3d vec3d = livingentity.getPositionEyes(1.0F);
              EntityBninz.EntityCustom.this.moveHelper.setMoveTo(vec3d.x, vec3d.y, vec3d.z, 1.0D);
            }
            
            public void updateTask() {
              EntityLivingBase livingentity = EntityBninz.EntityCustom.this.getAttackTarget();
              double d0 = EntityBninz.EntityCustom.this.getDistanceSq((Entity)livingentity);
              if (d0 <= getAttackReachSq(livingentity)) {
                EntityBninz.EntityCustom.this.attackEntityAsMob((Entity)livingentity);
              } else if (d0 < 16.0D) {
                Vec3d vec3d = livingentity.getPositionEyes(1.0F);
                EntityBninz.EntityCustom.this.moveHelper.setMoveTo(vec3d.x, vec3d.y, vec3d.z, 1.0D);
              } 
            ...
            protected double getAttackReachSq(EntityLivingBase attackTarget) {
              return EntityBninz.EntityCustom.this.width * 1.5D * EntityBninz.EntityCustom.this.height * 1.5D + attackTarget.height;
            }
          });
      this.tasks.addTask(2, (EntityAIBase)new EntityAIWander((EntityCreature)this, 0.8D, 20) {
            protected Vec3d getPosition() {
              Random random = EntityBninz.EntityCustom.this.getRNG();
              double dir_x = EntityBninz.EntityCustom.this.posX + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
              double dir_y = EntityBninz.EntityCustom.this.posY + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
              double dir_z = EntityBninz.EntityCustom.this.posZ + ((random.nextFloat() * 2.0F - 1.0F) * 16.0F);
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase target, float flval) {
      EntityBninz.EntityArrowCustom entityarrow = new EntityBninz.EntityArrowCustom(this.world, (EntityLivingBase)this);
